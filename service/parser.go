...
func argparseScheduleCommand(command []string) (time.Time, string, string, error) {
	...
	// argumentを定義
	channel := parser.String("c", "channel", &argparse.Options{Default: "", Help: "メッセージを送るチャンネル\n`#`からフルパスを記述してください\n省略した場合は予約メッセージを送信したチャンネルに送ります"})
	postTime := parser.String("t", "time", &argparse.Options{Required: true, Help: "メッセージを送る時間\nフォーマット: `yyyy/mm/dd/hh:mm`"})
	body := parser.String("b", "body", &argparse.Options{Required: true, Help: "送るメッセージ\nスペースが入るときは\"\"や''でくくって下さい\n改行したい場合は、\"\"や''でくくった上で改行したい箇所に`\\n`を挿入して下さい"})
	parser.DisableHelp()
